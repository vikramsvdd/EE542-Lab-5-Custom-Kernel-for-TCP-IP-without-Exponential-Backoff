...
static void tcp_incr_quickack(struct sock *sk, unsigned int max_quickacks)
{
	...
	if (quickacks > icsk->icsk_ack.quick)
		icsk->icsk_ack.quick = max_quickacks;// Lab 5 Change quickacks;
}
...
static void tcp_sndbuf_expand(struct sock *sk)
{
	...
	if (sk->sk_sndbuf < sndmem)
		WRITE_ONCE(sk->sk_sndbuf,READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_wmem[2])); //Lab 5 Changes;
			     ...
			     unsigned int skbtruesize)
{
	...
	while (tp->rcv_ssthresh <= window) {
		...
		truesize >>= 1;
		//window >>= 1;
		window=tp->rcv_ssthresh; //Lab 5 Changes
	}
	return 0;
}
			    ...
			    bool adjust)
{
	...
	if (room > 0 && !tcp_under_memory_pressure(sk)) {
		...
		if (incr) {
			...
			tp->rcv_ssthresh = tp->window_clamp; //Lab 5 Changes
...
__u32 tcp_init_cwnd(const struct tcp_sock *tp, const struct dst_entry *dst)
{
	...
	if (!cwnd)
		cwnd = TCP_INIT_CWND;
	return tp->snd_cwnd_clamp; //min_t(__u32, cwnd, tp->snd_cwnd_clamp); Lab 5 Changes
}
...
static void tcp_mtup_probe_success(struct sock *sk)
{
	...
	WARN_ON_ONCE((u32)val != val);*/ //Lab 5 change
